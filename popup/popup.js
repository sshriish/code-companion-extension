 ...
 * lightweight Settings tab for the GitHub PAT + Anthropic API key (MVP
...
async function loadSettings() {
  ...
  if (settings.anthropicKey) $("#pp-anthropic-key").placeholder = "•••••••••••••••• (saved)";
}
...
function setupSettingsForm() {
  $("#pp-settings-form").addEventListener("submit", async (e) => {
    ...
    const anthropicKey = $("#pp-anthropic-key").value.trim();

    const partial = {};
    if (githubToken) partial.githubToken = githubToken;
    if (anthropicKey) partial.anthropicKey = anthropicKey;
