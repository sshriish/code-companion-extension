/**
 * lib/groq-api.js
 * Wrapper for Groq's free-tier API (OpenAI-compatible chat completions
 * endpoint, running open-source models on Groq's LPU hardware). Only ever
 * invoked from the background service worker — the API key never reaches
 * a content script or the panel UI directly.
 *
 * Groq's free tier requires no credit card: sign up at console.groq.com,
 * generate a key, and you're within rate limits generous enough for this
 * extension's on-demand "Explain This" usage (see README for the current
 * limits — they're per-organization, not per-key).
 */
...
async function callGroq({ apiKey, system, prompt, maxTokens = 1200 }) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 429) {
      throw new Error(
        "Groq free-tier rate limit hit — wait a minute and try again (limits reset quickly)."
      );
    }
...
Respond with the JSON object described in your system instructions.`;
  ...
  const text = await callGroq({ apiKey, system: EXPLAIN_SYSTEM_PROMPT, prompt, maxTokens: 1200 });
...
Respond with the JSON object described in your system instructions.`;
  const text = await callGroq({ apiKey, system: SUMMARIZE_SYSTEM_PROMPT, prompt, maxTokens: 900 });
  return extractJson(text);
}
...
const EXPLAIN_COMMIT_SYSTEM_PROMPT = `You are a senior engineer reviewing a commit's diff for another developer who
wants to understand what changed and why, without reading the raw diff
line-by-line. Respond with STRICT JSON ONLY — no markdown fences, no
preamble. The JSON must match exactly this shape:

{
  "summary": "2-4 sentence plain-English summary of what this commit actually changes",
  "motivation": "1-2 sentence best guess at *why* this change was made, based on the commit message and the diff itself",
  "riskAreas": ["short description of a part of the diff worth double-checking, if any"]
}

If nothing in the diff looks risky, return an empty array for "riskAreas".
Do not include any text outside this JSON object.`;

export async function explainCommit({ apiKey, commitMessage, diff, repo }) {
...
Respond with the JSON object described in your system instructions.`;
  ...
  const text = await callGroq({
    apiKey,
    system: EXPLAIN_COMMIT_SYSTEM_PROMPT,
    prompt,
    maxTokens: 700
  });
  return extractJson(text);
