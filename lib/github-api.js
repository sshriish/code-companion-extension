...
/**
 * Search for code matching `query` inside a single repo, via GitHub's
 * code-search endpoint. Requires an authenticated token (code search
 * doesn't work unauthenticated). Returns raw hits — callers resolve exact
 * line numbers by fetching file content separately, since the search API
 * only reports byte offsets into a fragment, not real line numbers.
 */
export async function searchCode({ owner, repo, query, token, perPage = 10 }) {
  if (!token) {
    throw new Error("Code search needs a GitHub Personal Access Token (add one in Settings).");
  }
...
/**
 * Fetch a commit's unified diff as plain text via GitHub's `.diff` endpoint —
 * a lighter-weight alternative to walking `/commits/{sha}` file-by-file
 * through the REST API. Works for public repos with no token; private repos
 * need a token with repo access.
 */
export async function getCommitDiff({ owner, repo, sha, token }) {
  const url = `https://github.com/${owner}/${repo}/commit/${sha}.diff`;
  const headers = token ? { Authorization: `Bearer ${token}` } : {};
  const res = await fetch(url, { headers });
  if (!res.ok) {
    throw new Error(`Could not fetch commit diff (${res.status})`);
  }
  return res.text();
}
